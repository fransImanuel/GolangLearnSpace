...
func (d AccountRepositoryDb) Save(a Account)(*Account, *errs.AppError)  {
	sqlInsert := "INSERT INTO accounts (customer_id, opening_date, account_type, pin, status) values (?, ?, ?, ?, ?)"
	...
	if err!=nil {
		logger.Error("Error while Last Insert id forcreating new account : "+ err.Error())
		return nil, errs.NewNotFoundError("Unexpected  error from database")
	}
...
func (d AccountRepositoryDb) SaveTransaction(t Transaction) (*Transaction, *errs.AppError) {
	...
	if err!=nil {
		fmt.Println("ERROR-FRANS")
		fmt.Println(err)
	}

	// updating account balance
	if t.IsWithdrawal() {
		_, err = tx.Exec(`UPDATE accounts SET pin = pin - ? where account_id = ?`, t.Amount, t.AccountId)
	} else {
		_, err = tx.Exec(`UPDATE accounts SET pin = pin + ? where account_id = ?`, t.Amount, t.AccountId)
	}
...
func (d AccountRepositoryDb) FindBy(accountId string) (*Account, *errs.AppError) {
	sqlGetAccount := "SELECT account_id, customer_id, opening_date, account_type, pin from accounts where account_id = ?"
	var account Account
